...
paramsbs <- dftop %>%
  ...
  select(dist, var, value, bseason, movetype) %>%
  ggplot() +
  geom_point(aes(y = bseason, x = value, col = bseason, shape = movetype)) +
  facet_col(var~.,scales = "free", labeller = label_parsed) +
  scale_alpha(guide = "none") +
...
paramsnbs <- dftop %>%
  ...
  select(dist, var, value, wseason, movetype) %>%
  ggplot() +
  geom_point(aes(y = wseason, x = value, col = wseason, shape = movetype)) +
  facet_col(var~.,scales = "free", labeller = label_parsed) +
  scale_alpha(guide = "none") +
